...
def _restore_context(context: "contextvars.Context"):
...
def markcoroutinefunction(func: Callable[_P, _T]) -> Callable[_P, _T]:
...
class AsyncToSync(Generic[_P, _T]):
    """
    Utility class which turns an awaitable that only works on the thread with
    the event loop into a synchronous callable that works in a sub-thread.
    ...
    awaitable: Callable[_P, Awaitable[_T]]

    def __init__(self, awaitable: Callable[_P, Awaitable[_T]], force_new_loop: bool = False):
        if not callable(awaitable) or (
            not _iscoroutinefunction_or_partial(awaitable)
            and not _iscoroutinefunction_or_partial(getattr(awaitable, "__call__", awaitable))
        ):
    ...
    def __call__(self, *args: "_P.args", **kwargs: "_P.kwargs") -> _T:
        ...
        try:
            awaitable = self.main_wrap(
                call_result, source_thread, sys.exc_info(), context, *args, **kwargs
            )
    ...
    def _run_event_loop(self, loop: asyncio.AbstractEventLoop, coro: Coroutine[Any, Any, Any]):
        """
    ...
    async def main_wrap(
        self,
        call_result: Future[_T],
        source_thread: threading.Thread,
        exc_info: "OptExcInfo",
        context: Optional[List[contextvars.Context]],
        *args: "_P.args",
        **kwargs: "_P.kwargs",
    ):
        """
...
class SyncToAsync(Generic[_P, _T]):
    """
    ...
    thread-safe (for example, code which handles SQLite database connections).
    ...
    threadlocal: "threading.local" = threading.local()
    ...
    def thread_handler(
        ...
        source_task: Optional[asyncio.Task[Any]],
        exc_info: "OptExcInfo",
        func: Callable[..., Any],
        *args: Any,
        **kwargs: Any,
    ...
    @staticmethod
    def get_current_task() -> asyncio.Task[Any] | None:
        """
...
def sync_to_async(
    ...
    thread_sensitive: bool = True,
    executor: Optional["ThreadPoolExecutor"] = None,
) -> Callable[[Callable[_P, _T]], SyncToAsync[_P, _T]]:
    ...
...
def sync_to_async(
    ...
    thread_sensitive: bool = True,
    executor: Optional[ThreadPoolExecutor] = None,
) -> Callable[[Callable[_P, _T]], SyncToAsync[_P, _T]] | SyncToAsync[_P, _T]:
    if func is None:
